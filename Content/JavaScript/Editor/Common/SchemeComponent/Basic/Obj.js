...
class Obj extends React.Component {
    ...
    OnToggleFiledOptional = (key) => {
        ...
        if (fieldValue !== undefined) {
            ...
            const fieldTypeData = type.Fields[key];
    ...
    RenderFieldValue(fieldKey, fieldValue, fieldTypeData) {
        if (fieldTypeData.RenderType === 'array') {
            ...
            const isFolded = typeof value === 'object'
                ? value[fieldFoldKey]
                : false;
            return (React.createElement(Any_1.Any, { PrefixElement: React.createElement(CommonComponent_1.SlotText, { Text: fieldKey }), Value: fieldValue, Owner: value, Scheme: fieldTypeData, IsFolded: isFolded, OnFoldChange: (folded) => {
                    this.OnArrayFieldFoldChange(fieldKey, folded);
                }, OnModify: (v, type) => {
                    this.ModifyKv(fieldKey, v, type);
                } }));
        }
        return (React.createElement(Any_1.Any, { Value: fieldValue, Owner: this.props.Value, Scheme: fieldTypeData, OnModify: (obj, type) => {
