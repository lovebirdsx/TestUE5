...
import { TalkerCsvLoader, TalkerRow } from './TalkerCsv';

export const TALKER_LIST_CSV_PATH = '../../Config/Raw/Tables/d.对话人.csv';
    ...
    public static Load(): ITalkerListInfo {
        ...
        rows.forEach((row) => {
            talkers.push({
                Id: row.Id,
                Name: row.Name,
                CameraBindTag: row.CameraBindTag,
            });
            if (row.Id + 1 > maxId) {
                maxId = row.Id + 1;
            }
    ...
    public static Save(talkerList: ITalkerListInfo): void {
        ...
        talkerList.Talkers.forEach((talker) => {
            rows.push({
                Id: talker.Id,
                Name: talker.Name,
                CameraBindTag: talker.CameraBindTag,
            });
        });
